...
def calculate_norms(particle_data: dict):
    ...
    time = particle_data[number_of_particles[0]][kernels[0]][schemes[0]].metadata.t
    x, analytic = smooth_analytic_same_api_as_swiftsimio(time)
        ...
        for kernel in kernels:
            ...
            for scheme in schemes:
                ...
                for property in analytic.keys():
                    if property == "velocities_phi":
                        x = coords[:, 0]
                        y = coords[:, 1]

                        vel = this_data.gas.velocities.value
                        v_phi = (-y * vel[:, 0] + x * vel[:, 1])[mask] / radii

                        analytic_velocity = analytic["velocities_phi"](radii)

                        L1 = L1_norm(v_phi, analytic_velocity)
                        L2 = L2_norm(v_phi, analytic_velocity)

                        this_output["velocities_phi"] = [L1, L2]
                    else:
